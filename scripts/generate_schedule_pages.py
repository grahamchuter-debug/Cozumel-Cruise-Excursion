...
DATA = ROOT / "data" / "generated" / "cozumel-schedules.json"
...
<section class="schedule-hub pt-24 pb-16">
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <h1 class="text-3xl sm:text-4xl font-display font-bold text-gray-900 mb-4">Cozumel cruise ship schedule</h1>
    <p class="text-gray-600 leading-relaxed mb-4 max-w-2xl">
      Find your ship and date for Cozumel, then plan a realistic port day around reef time, a beach club or island sightseeing.
      This schedule is synced from the Caribbean Shore Excursions authority dataset for Cozumel only.
    </p>
    {disclaimer()}
    ...
    <div class="bg-sand-50 rounded-2xl p-6 border border-pr-100 mb-10">
      ...
      <p class="text-sm text-gray-600 mt-3"><a class="text-ocean-600 font-semibold" href="../cozumel-port-guide.html">Port &amp; terminal guide</a> · <a class="text-ocean-600 font-semibold" href="../best-cozumel-shore-excursions.html">Excursion options</a> · <a class="text-ocean-600 font-semibold" href="../one-day-in-cozumel-from-a-cruise-ship.html">One-day planning</a></p>
    </div>
...
<section class="schedule-hub pt-24 pb-16">
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <h1 class="text-3xl sm:text-4xl font-display font-bold text-gray-900 mb-3">Cozumel cruise schedule {year}</h1>
...
<section class="schedule-hub pt-24 pb-16">
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <h1 class="text-3xl sm:text-4xl font-display font-bold text-gray-900 mb-2">{label} — Cozumel cruise calls</h1>
    ...
    <p class="mt-10 text-sm flex flex-wrap gap-4">
      ...
      <a class="text-ocean-600 font-semibold" href="../../../cozumel-port-guide.html">Port guide</a>
      <a class="text-ocean-600 font-semibold" href="../../../best-cozumel-shore-excursions.html">Excursions</a>
    </p>
  </div>
</section>
"""
...
def main() -> list[tuple[str, str, str]]:
    if not DATA.exists():
        raise SystemExit(f"Missing {DATA} — run npm run sync:schedules first")
    ...
    write(
        hub_dir / "index.html",
        page_shell(
            title="Cozumel Cruise Ship Schedule | Find Your Ship &amp; Date",
            description="Cozumel cruise ship schedule for cruise passengers — find your date and ship, then plan reef, beach or island excursions around your port call.",
    ...
    for y in years:
        write(
            ...
            page_shell(
                title=f"Cozumel Cruise Schedule {y} | Ship Calls by Month",
                description=f"Cozumel cruise ship schedule for {y} — {by_year[y]} scheduled calls. Browse populated months and search by ship or date.",
    ...
    for ym, month_calls in by_month.items():
        ...
        write(
            ...
            page_shell(
                title=f"{label} Cozumel Cruise Schedule | {len(month_calls)} Ship Calls",
                description=f"Cozumel cruise ship arrivals in {label} — {len(month_calls)} scheduled calls with ship names, cruise lines and planned arrival/departure times.",
